...
class Products(Resource):
    ...
    def post(self):
        ...
        cur.execute("SELECT * FROM products WHERE name='{}';".format(json_data['name']))
    ...
    def get(self):
        ...
        """
        store_id = get_store_id(get_jwt_identity())
        cur.execute("SELECT * FROM products WHERE store_id={};".format(store_id))
...
class ProductDetail(Resource):
    ...
    def post(self,id):
        ...
        if product[3] < int(number):
            msg = 'There are only {0} {1} available '.format(product[3],product_name)
            abort(406,msg)
        ...
        cur.execute("UPDATE products SET inventory={0} WHERE id ={1}".format(new_inventory,id))
