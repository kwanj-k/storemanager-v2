...
class Carts(Resource):
    ...
    def post(self):
        ...
        for c in cart:
            ...
            number = c[3]
            amount = c[4]
            new_sale_record = Sale(store_id,seller_id,product,number,amount)
    ...
    def delete(self):
        ...
        for c in cart:
            inventory = c[3]
            name = c[2]
            cur.execute("UPDATE products SET inventory= inventory + '{}' WHERE name ='{}'".format(inventory,name))
        cur.execute("DELETE FROM carts WHERE seller_id={};".format(seller_id))
...
class CartDetail(Resource):
    ...
    def put(self,id):
        ...
        if not product or product[1] != seller_id:
            ...
            abort(404,msg)
        cur.execute("SELECT * FROM products WHERE name='{}';".format(product[2]))
        p = cur.fetchone()
        ...
        if p[3] < int(number):
            ...
            abort(406,msg)
        new_amnt = number * p[4]
        cur.execute("UPDATE carts SET number={0},amount={1} WHERE id ={2}".format(number,new_amnt,id))
        conn.commit()
        new_p_inv = product[3] - int(number)
        cur.execute("UPDATE products SET inventory= inventory + '{}' WHERE name ='{}'".format(new_p_inv,product[2]))
        conn.commit()
        cur.execute("SELECT * FROM carts WHERE id={};".format(id))
        new_c = cur.fetchone()
        format_new_c ={
